...
public class AppSettings {
    ...
    public static boolean isFirstInstall(Context context) {
        return context.getSharedPreferences(settings_file, 0).getBoolean(FIRST_INSTALL, true);
    }

    public static void putFirstInstall(Context context, boolean value) {
        context.getSharedPreferences(settings_file, 0).edit().putBoolean(FIRST_INSTALL, value).commit();
    }

    public static int getAppVersion(Context context) {
        return context.getSharedPreferences(settings_file, 0).getInt(CURRENT_VERSION, Integer.MAX_VALUE);
    }
    ...
    public static void putSelectedFeature(Context context, AppFeature feature, int id) {
        context.getSharedPreferences(settings_file, 0).edit().putInt(SELECTED_FEATURE, feature.getId()).commit();
        context.getSharedPreferences(settings_file, 0).edit().putInt(SELECTED_FEATURE_ID, id).commit();
    }

    public static Pair<AppFeature, Integer> getSelectedFeature(Context context) {
        AppFeature feature = AppFeature.getFeatureForId(context.getSharedPreferences(settings_file, 0).getInt(SELECTED_FEATURE, 0));
        int id = context.getSharedPreferences(settings_file, 0).getInt(SELECTED_FEATURE_ID, -1);
        return new Pair<>(feature, id);
    }

    public static void putDefaultFeature(Context context, AppFeature feature, int id) {
        context.getSharedPreferences(settings_file, 0).edit().putInt(DEFAULT_FEATURE, feature.getId()).commit();
...
//        return new Pair<AppFeature, Integer>(feature, id);
    ...
    public static boolean isDailyNotificationEnabled(Context context) {
        return context.getSharedPreferences(settings_file, 0).getBoolean("votd_enabled", false);
    }

    public static boolean isScheduledNotificationEnabled(Context context) {
        return context.getSharedPreferences(settings_file, 0).getBoolean("schedule_enabled", false);
    }

    public static void putTime(Context context, String key, int hour, int minute) {
        context.getSharedPreferences(settings_file, 0).edit().putInt(TIME_HOUR + "_" + key, hour).commit();
        context.getSharedPreferences(settings_file, 0).edit().putInt(TIME_MINUTE + "_" + key, minute).commit();
    }

    public static Pair<Integer, Integer> getTime(Context context, String key) {
        int hour = context.getSharedPreferences(settings_file, 0).getInt(TIME_HOUR + "_" + key, 8);
        int minute = context.getSharedPreferences(settings_file, 0).getInt(TIME_MINUTE + "_" + key, 0);
