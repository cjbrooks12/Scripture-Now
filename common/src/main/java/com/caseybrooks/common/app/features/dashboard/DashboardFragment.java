...
//--------------------------------------------------------------------------------------------------
    ...
    private void createFeatureViews() {
        ...
        for(Pair<Class<? extends FeatureConfiguration>, Boolean> feature : getActivityBase().getAppFeatures()) {
            try {
                FeatureConfiguration featureConfiguration = Util.findFeatureConfiguration(getContext(), feature.first);

                if(featureConfiguration != null) {
                    DashboardCardBase card =
                            featureConfiguration
                                    .getDashboardCardConfiguration(getContext())
                                    .getCardClass()
        ...
        Collections.sort(features, new Comparator<DashboardCardBase>() {
            ...
            public int compare(DashboardCardBase lhs, DashboardCardBase rhs) {
                return lhs.getConfiguration().getRelativePosition() - rhs.getConfiguration().getRelativePosition();
            }
        });
    }
    ...
    public FragmentConfiguration configuration = new FragmentConfiguration() {
        ...
        public DrawerFeature getDrawerFeature() {
            return new DrawerFeature(DashboardFeatureConfiguration.getInstance(getContext()), getTitle(), R.drawable.ic_home);
        }
...
//--------------------------------------------------------------------------------------------------
    ...
    private class DashboardAdapter extends RecyclerView.Adapter<DashboardViewHolder> implements ItemTouchHelperAdapter {
        ...
        public DashboardViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            // find the card corrseponding to the viewtype
            for (DashboardCardBase card : features) {
                if(card.getConfiguration().getViewType() == viewType) {
        ...
        public int getItemViewType(int position) {
            return features.get(position).getConfiguration().getViewType(); //forces every item in the list to be its own view
        }
