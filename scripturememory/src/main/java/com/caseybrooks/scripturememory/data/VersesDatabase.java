	...
	public Passage getEntryAt(long id) throws SQLException {
		...
		if(c.getCount() > 0) {
            try {
                ...
                Passage passage = new Passage(c.getString(iReference), c.getString(iVerse));

                passage.addTag(c.getString(iList));
                passage.setId(Integer.parseInt(c.getString(iRow)));
                return passage;
            }
            catch (IllegalArgumentException e) {
                throw new SQLException("Verse with id '" + id + ": " + c.getString(iReference) + "' is not formatted properly");
            }
		}
		else throw new SQLException("Cannot find Verse with id '" + id + "'");
	}
...
//------------------------------------------------------------------------------
	...
	public void importFromCSV(File filename) throws SQLException, IOException {
		...
		Passage verse = getFirstEntry("current");

        MetaSettings.putVerseId(context, verse.getId());
	}
}
