...
public class VOTDCard extends LinearLayout {
//Data Members
//------------------------------------------------------------------------------
	Context context;
		...
		AlertDialog.Builder builder = new AlertDialog.Builder(context);
			...
			builder.setPositiveButton("Save", new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int id) {
					boolean success;
					try {
						VersesDatabase entry = new VersesDatabase(getContext());
						entry.open();
						entry.createEntry(ref.getText().toString(), ver.getText().toString(), "current");
						entry.close();
						success = true;
					}
					catch(Exception ex) {
